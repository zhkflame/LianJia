...
class SQLiteWraper(object):
    ...
    def execute(self,command,method_flag=0,conn=None):
        cu = conn.cursor()
        try:
            c=cu.execute("PRAGMA table_info([xiaoqu])")
            print c
            if not method_flag:
                cursor=cu.execute(command)
            else:
...
def test_command(info_dict):
    """
    生成小区数据库插入命令
    """
    command=(r"select * from xiaoqu")
    return command
...
if __name__=="__main__":
    ...
    command1="select * from xiaoqu limit 10"
    db_xq.execute(command1)
